...
public class BoardInfoService {
    ...
    public ListData<BoardData> getList(BoardDataSearch search) {
        ...
        if(StringUtils.hasText(skey)) {
            ...
            } else if (skey.equals("poster")) { // 작성자 + 아이디 + 회원명
                BooleanBuilder orBuilder = new BooleanBuilder();
        ...
        int total = (int) boardDataRepository.count(andBuilder); // 게시판 전체 개수
    ...
    public boolean isMine(Long seq) {
        ...
        if (data.getMember() != null) {
            ...
            String key = "chk_" + seq;
            if (session.getAttribute(key) == null) { // 비회원 비밀번호 검증X => 검증 화면 으로 이동
    ...
    public boolean checkGuestPassword(Long seq, String password) {
        ...
        if (!StringUtils.hasText(guestPw)) { // 값이 없을 때
            return false;
        }

        return encoder.matches(password, guestPw); // 입력받은 pw와 비교
    }
}