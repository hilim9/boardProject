...
public class CommentInfoService {
    ...
    public void isMine(Long seq) {
        if(memberUtil.isAdmin()) { // 관리자일 때 통과
            return;
        }
    ...
    public boolean isEditable(CommentData comment) {
        ...
        // 관리자이거나 비회원 댓글이면 무조건 노출
        if (memberUtil.isAdmin() || commentMember == null) {
            return true;
        }

        // 회원 댓글이면 직접 작성한 댓글만 노출
        if (memberUtil.isLogin() && commentMember != null
                && commentMember.getUserNo().longValue()
                == memberUtil.getMember().getUserNo().longValue()) {
